...
- `@neon/admin-crud`: `parseListQuery` (`limit`/`skip` query params only; response `meta` is `{ total, limit, skip }`), `buildFilterConditions` suffix operators (`_in`, `_not`, `_gte`, ...), `bulkProvider` (`POST/PATCH /bulk`), column-derived ArkType list schemas; `crudProvider` still valid for table-only resources. Admin SPA uses UI `page`/`pageSize` and converts to `limit`/`skip` at the HTTP boundary.
