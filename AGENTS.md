...
- `@neon/admin-crud`: `parseListQuery`, `buildFilterConditions` suffix operators (`_in`, `_not`, `_gte`, ...), `bulkProvider` (`POST/PATCH /bulk`), column-derived ArkType list schemas; `crudProvider` still valid for table-only resources.
